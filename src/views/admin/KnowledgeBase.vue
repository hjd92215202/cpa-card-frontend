<template>
  <div class="flex h-screen bg-gray-100 overflow-hidden text-gray-700 font-sans">
    ...
    <div :class="[
      ...
      <!-- 顶部控制区 -->
      <div class="p-4 flex items-center border-b border-slate-800 overflow-hidden"
        :class="isCollapsed ? 'justify-center' : 'justify-between'">
        <div v-show="!isCollapsed"
          class="flex items-center gap-2 cursor-pointer hover:text-indigo-400 transition-all group whitespace-nowrap"
          @click="router.push('/')">
          <el-icon class="group-hover:scale-110 transition-transform" :size="20">
            <HomeFilled />
          </el-icon>
          <h1 class="font-black text-sm tracking-widest uppercase">返回首页</h1>
        </div>

        <el-button link :icon="isCollapsed ? Expand : Fold" class="text-slate-400 hover:text-white"
      ...
      <div class="flex-1 overflow-y-auto py-2 custom-scrollbar overflow-x-hidden">
        <div v-for="sub in subjects" :key="sub.id" @click="selectSubject(sub)" :class="[
          ...
          <el-icon v-show="!isCollapsed"
            class="opacity-0 group-hover:opacity-100 hover:text-red-400 transition-all ml-2" :size="14"
      ...
      <div class="p-4 border-t border-slate-800 bg-slate-950/20 flex flex-col items-center gap-4">
        <div
          :class="['flex items-center w-full overflow-hidden transition-all', isCollapsed ? 'justify-center' : 'gap-3 justify-between']">
    ...
    <div class="w-80 bg-white border-r flex flex-col shadow-sm">
      ...
      <div class="flex-1 overflow-y-auto p-2 custom-scrollbar">
        <el-tree v-if="appStore.currentSubjectId" :data="categoryTree" node-key="id" @node-click="handleNodeClick"
          ...
          <template #default="{ data }">
            <div class="flex-1 flex items-center justify-between text-sm group pr-2 truncate">
              ...
              <div class="flex gap-1">
                <el-icon class="hidden group-hover:block text-indigo-500 hover:scale-125 transition-transform"
        ...
        <div v-else
          class="h-full flex flex-col items-center justify-center text-gray-400 text-xs px-10 text-center space-y-3 opacity-60">
          <el-icon :size="32">
            <Files />
          </el-icon>
          <p>请选择左侧科目</p>
    ...
    <div class="flex-1 flex flex-col bg-gray-50 overflow-hidden">
      <header v-if="currentCategoryId"
        class="bg-white p-6 border-b flex justify-between items-center shadow-sm relative z-10">
        <div>
          <div class="flex items-center gap-2">
            <h2 class="text-xl font-bold text-gray-800">{{ currentCategoryName }}</h2>
            <el-tag size="small" effect="plain" round class="bg-indigo-50 border-indigo-100 text-indigo-600">{{
              cards.length
              }} 卡片</el-tag>
          </div>
        ...
        <div class="flex gap-3">
          ...
          <el-button type="success" :icon="Postcard" class="bg-green-600 border-none hover:bg-green-500"
            @click="goToStudy">复习模式</el-button>
          <el-button type="primary" :icon="Plus" class="bg-indigo-600 border-none hover:bg-indigo-500"
            @click="openCardEditor">新建卡片</el-button>
        </div>
      </header>
      ...
      <main class="flex-1 overflow-y-auto p-6 custom-scrollbar">
        <div v-if="currentCategoryId">
          <div v-if="cards.length > 0" class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
            <el-card v-for="card in cards" :key="card.id" shadow="hover"
              ...
              <template #header>
                <div class="flex justify-between items-center">
                  <div class="flex items-center gap-2 truncate">
                    <el-icon v-if="card.card_type === 'code'" class="text-orange-500">
                      <Monitor />
                    </el-icon>
                  ...
                  <div class="flex items-center gap-2">
                    <el-button class="hidden group-hover:block" link type="danger" :icon="Delete"
              ...
              <div class="mt-4 pt-4 border-t flex justify-between items-center">
                ...
                <span class="text-[10px] text-gray-400 font-mono tracking-tighter">NEXT: {{ card.next_review_date ||
                  'New'
                  }}</span>
              </div>
            </el-card>
          ...
          <div v-else
            class="h-64 flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-2xl bg-white">
            <el-icon class="text-gray-200" :size="48">
              <Postcard />
            </el-icon>
        ...
        <div v-else class="h-full flex flex-col items-center justify-center text-gray-400 animate-pulse space-y-4">
          <el-icon :size="64" class="opacity-10">
            <Memo />
          </el-icon>
    ...
    <!-- 弹窗：知识卡片编辑器 (核心修复点) -->
    <el-dialog v-model="showCardEditor" :title="editingCardId ? '更新知识节点' : '录入新知识点'" width="85%" top="5vh"
      destroy-on-close>
      <el-form :model="cardForm" label-position="top">
        <div class="grid grid-cols-12 gap-8">
          ...
          <div class="col-span-4 bg-gray-50 p-6 rounded-xl space-y-8 border border-gray-100">
            ...
            <div class="pt-10">
              <el-button type="primary" size="large" class="w-full shadow-lg bg-indigo-600 border-none"
